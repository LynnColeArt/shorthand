# Windows ISAPI Installation under IIS
Current version of ShortHand \(1.2\) and higher support Microsoft Internet Information Server \(IIS\) in ISAPI mode in addition to CGI mode. ISAPI is preferred to CGI for reasons of speed and flexibility. The ISAPI implementation is also much lighter on server resources than it's CGI predecessor. To install ShortHand ISAPI under IIS, follow these steps: 
  ...
  2. Place file **shorthand\_ISAPI.dll** into separate directory that is **not** anywhere under your documents hierarchy. For example, `C:\ShortHand`.  
