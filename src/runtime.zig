...
pub const Runtime = struct {
    ...
    fn cloneValueToAllocator(self: *Runtime, allocator: std.mem.Allocator, v: value.Value) !value.Value {
        return switch (v) {
            .none => .{ .none = {} },
            .boolean => |b| .{ .boolean = b },
            .integer => |i| .{ .integer = i },
...
const Runner = struct {
    ...
    fn compareValues(self: *Runner, lhs: value.Value, rhs: value.Value, strict: bool) anyerror!std.math.Order {
        if (!strict) {
            if (lhs == .string or lhs == .cookie or lhs == .object or rhs == .string or rhs == .cookie or rhs == .object) {
                const l = switch (lhs) {
                    .string => lhs.string,
                    .cookie => lhs.cookie.value,
                    .object => "",
                    else => "",
                };
                const r = switch (rhs) {
                    .string => rhs.string,
                    .cookie => rhs.cookie.value,
                    .object => "",
                    else => "",
                };
    ...
    fn parseAssignment(self: *Runner, execute: bool, writer: anytype) anyerror!Flow {
        ...
        self.skipEols();
        if (execute) {
            try self.runtime.setVariable(name, assigned_value);
        }
        return .normal;
    }
    ...
    fn popScopeTo(self: *Runner, mark: usize) void {
        while (self.runtime.variables.items.len > mark) {
            ...
            if (binding.value == .string) {
                self.runtime.allocator.free(binding.value.string);
            }
